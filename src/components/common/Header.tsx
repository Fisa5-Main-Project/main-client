...
interface HeaderProps {
  ...
  /** 로고(또는 타이틀) 노출 여부 (기본값: true) - 현재는 타이틀만 제어 */
  hasLogo?: boolean;
  /** 홈 버튼 노출 여부 (기본값: true) */
  hasHome?: boolean;
...
export default function Header({
  ...
  return (
    <header className="flex items-center justify-between h-[3.75rem] shrink-0 border-b border-gray-100/50 backdrop-blur-sm px-6">
      {/* 1. 왼쪽: 뒤로가기 버튼 */}
      <div className="w-10 flex justify-start ">
        ...
        {hasBackButton && (
          <button
            type="button"
            onClick={() => router.back()}
            className="w-10 h-10 flex items-center justify-center cursor-pointer hover:bg-black/5 rounded-full transition-colors -ml-8"
            aria-label="뒤로가기"
          >
            <ChevronLeftIcon />
          </button>
      ...
      <div className="flex justify-end items-center gap-4">
        ...
        {hasMyPage && (
          <Link
            href={PATHS.MYPAGE}
            className="w-10 h-10 flex items-center justify-center cursor-pointer hover:opacity-80 transition-opacity -mr-10"
            aria-label="마이페이지"
          >
